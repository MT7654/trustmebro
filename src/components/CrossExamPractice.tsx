...
export const CrossExamPractice:React.FC<Props>=({isOpen,onComplete,onSkip,isReducedMotion=false})=>{
  ...
  return <div id="cross-exam-practice-root" className="fixed inset-0 z-[60] flex flex-col overflow-hidden bg-slate-950 p-2 sm:p-4" role="dialog" aria-modal="true" aria-label="Interactive cross-examination practice">
    <div className="mb-2 flex shrink-0 items-center justify-between"><div><p className="text-[10px] font-display font-black uppercase tracking-[.22em] text-cyan-300">Isolated practice · nothing is saved</p><h2 className="font-heading text-xl font-black">Test a claim on the real controls</h2></div><button onClick={()=>setIsSkipConfirmOpen(true)} className="flex items-center gap-2 rounded-full border border-white/20 px-4 py-2 text-xs font-bold uppercase"><SkipForward className="h-4 w-4"/>Skip tutorial</button></div>
    ...
    {config&&<GuidedSpotlight target={config.target} scopeSelector="#cross-exam-practice-root" title={config.title} instruction={config.instruction} step={stepIndex+1} total={order.length} onSkip={onSkip} isReducedMotion={isReducedMotion} skipDestination="cross-examination"/>}
    <TutorialSkipConfirm isOpen={isSkipConfirmOpen} destination="cross-examination" onCancel={()=>setIsSkipConfirmOpen(false)} onConfirm={()=>{setIsSkipConfirmOpen(false);onSkip();}} isReducedMotion={isReducedMotion}/>
  </div>;
};
